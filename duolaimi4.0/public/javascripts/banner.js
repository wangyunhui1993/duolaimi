$(function(){
	...
	function slide(slideMode){//轮播方法		
		if (isMoving==false){
			...
			if(slideMode){			//slideMode为1(true)，执行slide(1)，上一张
				...
				$(".main_box li").find("p").css({"bottom":"-50px"});//所有标题隐藏
				...
				//所有标题隐藏
				$(".main_box li").find("p").css({"bottom":"-50px"});
	...
	if(len>3){
		...
		$(".btn_list span").click(function(event){
			...
			if(Math.abs(index_2-oIndex)>1&&Math.abs(len-Math.abs(index_2-oIndex))!=1){
				//统一样式
				$(".main_box li").css({width:"300px",height:"120px",left:"600px",top:"60px",opacity:0});
	...
	//鼠标移入为当前正中显示的图片li，则清除定时器
		$(".main_box li").on("mousemove",function(){
